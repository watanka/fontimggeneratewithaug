...
def random_number(digit):
    bill = []
    digit_num = np.random.randint(1, digit)
    period_num = np.random.randint(0, digit_num)
    add_comma = random.randint(0, 9)
    add_period = np.random.randint(0, 2)
    for digit in range(1, digit_num + 1):
        add_punct = random.randint(0, 2)
        num = str(np.random.randint(0, 9))
        if digit == digit_num:
            num = str(random.randint(1, 9))
        bill.append(num)

        if add_comma > 6:
            if len([_ for _ in bill if _ != ","]) % 3 == 0 and digit != digit_num:
                bill.append(",")
        if not add_comma and add_period and digit == period_num:
            bill.append(".")
        if not add_comma and not add_period and add_punct > 1:
            bill.append(random.choice(["-", ":", "(", ")", "#", "/"]))
